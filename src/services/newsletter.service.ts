...
export async function getOrCreateDefaultListId(): Promise<number> {
...
export class NewsletterService {
    async subscribe(input: {
        ...
        email: string;
        source?: string;
    }): Promise<{ ok: true; alreadySubscribed: boolean } | { ok: false }> {
        ...
        try {
            ...
            const existing = await db.query.marketingSubscribers.findFirst({
                where: and(
                    eq(marketingSubscribers.listId, listId),
                    eq(marketingSubscribers.email, normalizedEmail),
                ),
            });
    ...
    async unsubscribe(token: string): Promise<boolean> {
        ...
        try {
            const row = await db.query.marketingSubscribers.findFirst({
                where: and(
                    eq(marketingSubscribers.unsubscribeToken, trimmed),
