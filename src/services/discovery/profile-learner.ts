...
import { LEARNING_DELTAS, type DiscoveryEventType } from './weights.js';
...
export async function listingInterestTags(productId: number): Promise<string[]> {
    const [row] = await db
        .select({
    ...
    const tags: string[] = [];
    if (row.categoryName) tags.push(slugTag('category', row.categoryName));
    ...
    for (const name of ['gmail', 'slack', 'whatsapp', 'shopify', 'stripe', 'microsoft']) {
        if (integ.includes(name)) tags.push(slugTag('integration', name));
    }
...
export async function applyEventToInterestProfile(
    clientId: number,
    productId: number,
    ...
    if (eventType === 'impression' || eventType === 'search') return null;
    ...
    if (tags.length === 0 && eventType !== 'unsave') return null;
