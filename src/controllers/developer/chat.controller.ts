...
import {
    ...
    streamClientUserId,
    streamDeveloperUserId,
} from '../../services/stream-chat.service.js';
...
export class DeveloperChatController {
    async getToken(c: Context) {
        if (!isStreamChatConfigured()) return notConfigured(c);

        const jwtUser = c.get('user') as JWTPayload | undefined;
    ...
    async postThread(c: Context) {
        ...
        try {
            const [link] = await db
                ...
                .where(
                    and(
                        eq(productReviews.clientId, clientId),
