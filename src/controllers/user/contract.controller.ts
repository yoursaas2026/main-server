...
export class UserContractController {
    ...
    async create(c: Context) {
        ...
        if (Number.isNaN(deadline.getTime())) return c.json({ success: false, error: 'Invalid deadline' }, 400);
    ...
    async payAmendment(c: Context) {
        const u = c.get('user') as JWTPayload | undefined;
        if (!u || u.role !== 'client') return c.json({ success: false, error: 'Unauthorized' }, 401);
        const publicId = c.req.param('publicId');
        const amendmentId = parseInt(c.req.param('amendmentId') || '', 10);
        if (!Number.isInteger(amendmentId)) return c.json({ success: false, error: 'Invalid amendment' }, 400);
        try {
    ...
    async acceptCompletion(c: Context) {
        const u = c.get('user') as JWTPayload | undefined;
        if (!u || u.role !== 'client') return c.json({ success: false, error: 'Unauthorized' }, 401);
        const publicId = c.req.param('publicId');
        try {
    ...
    async requestRevision(c: Context) {
        const u = c.get('user') as JWTPayload | undefined;
        if (!u || u.role !== 'client') return c.json({ success: false, error: 'Unauthorized' }, 401);
        const publicId = c.req.param('publicId');
    ...
    async openDispute(c: Context) {
        const u = c.get('user') as JWTPayload | undefined;
        if (!u || u.role !== 'client') return c.json({ success: false, error: 'Unauthorized' }, 401);
        const publicId = c.req.param('publicId');
        const body = await c.req.json().catch(() => null);
        const reason = typeof body?.reason === 'string' ? body.reason.trim() : '';
        if (reason.length < 10) return c.json({ success: false, error: 'Please provide a clear reason (10+ chars).' }, 400);
        try {
