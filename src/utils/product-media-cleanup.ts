...
export function isProductOwnedMediaUrl(ref: string): boolean {
    const u = ref.trim();
    return PREFIXES.some((p) => u.startsWith(p));
}

function resolvedPathStaysUnderProducts(absFile: string): boolean {
...
export function productMediaUrlToAbsolutePath(ref: string): string | null {
    const u = ref.trim();
    if (!isProductOwnedMediaUrl(u)) return null;
    const rel = u.replace(/^\/+/, '');
    const abs = path.resolve(process.cwd(), rel);
    if (!resolvedPathStaysUnderProducts(abs)) return null;
    return abs;
}

export function deleteProductMediaFile(url: string): void {
...
export function parseScreenshotUrlsColumn(raw: string | null): string[] {
    if (!raw) return [];
    try {
...
export function cleanupReplacedProductMedia(
    previous: { iconUrl: string | null; screenshotUrls: string | null },
    next: { iconUrl: string; screenshotUrls: string[] }
): void {
...
export function cleanupAllProductMediaForRow(row: { iconUrl: string | null; screenshotUrls: string | null }): void {
    const icon = (row.iconUrl ?? '').trim();
    if (icon) deleteProductMediaFile(icon);
    for (const u of parseScreenshotUrlsColumn(row.screenshotUrls)) {
        const t = u.trim();
        if (t) deleteProductMediaFile(t);
    }
}
