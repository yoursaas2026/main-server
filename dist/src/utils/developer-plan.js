...
export function normalizeDeveloperPlan(plan) {
 ...
 */
export function effectiveDeveloperPlan(plan, planEndDate, now = new Date()) {
...
export function isDeveloperPaidPlanExpired(plan, planEndDate, now = new Date()) {
    const normalized = normalizeDeveloperPlan(plan);
    if (normalized === 'base')
        return false;
    if (!planEndDate)
...
export async function ensureDeveloperPlanNotExpired(developerId) {
    const [row] = await db
        .select({
...
export async function downgradeExpiredDeveloperPlans(now = new Date()) {
    ...
    await db
        ...
        .where(and(inArray(developers.id, expired.map((r) => r.id)), ne(developers.plan, 'base')));
