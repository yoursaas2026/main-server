...
export function getMissingForLiveListing(input) {
    ...
    if (goodBenefits.length < 1)
        missing.push('At least 1 key benefit with title and description');
    const goodFeatures = (input.features || []).filter((f) => f.title?.trim().length >= 2 &&
        f.short?.trim().length >= 8 &&
        f.detail?.trim().length >= 20);
    if (goodFeatures.length < 1)
        missing.push('At least 1 feature with title, short line, and full detail');
    const cases = (input.useCases || []).map((u) => u.trim()).filter(Boolean);
    if (cases.length < 1)
        missing.push('At least 1 use case');
    const tags = (input.audienceTags || []).map((t) => t.trim()).filter(Boolean);
    if (tags.length < 1)
        missing.push('At least 1 audience tag');
