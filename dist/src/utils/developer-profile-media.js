...
export function isDeveloperProfileMediaUrl(ref) {
    if (!ref?.trim())
        return false;
    const u = ref.trim();
    return u.startsWith(UPLOAD_PREFIX) || u.startsWith(`public${UPLOAD_PREFIX}`);
}
export function developerProfileMediaToAbsolutePath(ref) {
    const u = ref.trim();
    if (!isDeveloperProfileMediaUrl(u))
        return null;
    const rel = u.replace(/^\/+/, '').replace(/^public\//, '');
    const abs = path.resolve(process.cwd(), rel);
    const uploadRoot = path.resolve(process.cwd(), 'uploads', 'developer_profiles');
    const norm = path.normalize(abs);
    if (norm !== uploadRoot && !norm.startsWith(uploadRoot + path.sep))
        return null;
    return norm;
}
export function deleteDeveloperProfileMedia(ref) {
    if (!ref || !isDeveloperProfileMediaUrl(ref))
        return;
...
async function compressImageBuffer(buffer, kind) {
    try {
        const pipeline = sharp(buffer).rotate();
        if (kind === 'profile') {
            return Buffer.from(await pipeline.resize(512, 512, { fit: 'cover', withoutEnlargement: true }).jpeg({ quality: 85 }).toBuffer());
        }
        return Buffer.from(await pipeline
...
export async function saveDeveloperProfileImage(file, kind, developerId) {
