 ...
 */
export function scoreListingCandidate(listing, buyer, opts) {
    const breakdown = {};
    const reasons = [];
    let total = 0;
    ...
    const audienceHits = overlapCount([buyer.industry || '', buyer.companySize || ''].filter(Boolean), listing.audienceTags.join(' ').toLowerCase() + ' ' + hay);
    if (audienceHits > 0) {
    ...
    if (buyer.companySize && hay.includes(buyer.companySize.toLowerCase().replace(/_/g, ' '))) {
        ...
        const goalHits = buyer.primaryGoals.filter((g) => hay.includes(g.replace(/_/g, ' ')) || hay.includes(g)).length;
        if (goalHits > 0) {
            breakdown.goals = 15;
            total += 15;
    ...
    if (listing.hasIcon)
        quality += 2;
    if (listing.screenshotCount > 0)
        quality += 2;
    if (listing.hasSupportEmail)
        quality += 2;
    if (listing.hasTiers)
        quality += 2;
    if (quality > 0) {
    ...
    const maxPossible = SCORE_MAX_WITHOUT_SEMANTIC + (breakdown.semantic ? SCORING_WEIGHTS.semanticMax : 0) + 15; // goals bonus
