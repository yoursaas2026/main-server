...
function normalizeTiers(raw) {
    ...
    return ['base', 'plus', 'pro'].map((id) => {
        const row = byId.get(id);
        if (!row)
            return defaults[id];
        const priceRaw = row.fixedPriceInr ?? row.priceInr ?? row.price;
        let fixedPriceInr = defaults[id].fixedPriceInr;
        if (priceRaw === null || priceRaw === 'null')
            fixedPriceInr = null;
        else if (priceRaw !== undefined)
            fixedPriceInr = Math.max(0, asInt(priceRaw, defaults[id].fixedPriceInr ?? 0));
        return {
            id,
...
export function normalizeAstraPayload(raw, categoryNames, productNameHint) {
    const obj = raw && typeof raw === 'object' ? raw : {};
    const nested = obj.listing && typeof obj.listing === 'object'
        ? obj.listing
    ...
    const categoryName = asString(nested.categoryName || nested.category || nested.productCategory, defaultCategory) || defaultCategory;
    const benefitsRaw = Array.isArray(nested.benefits) ? nested.benefits : [];
    const benefits = padBenefits(benefitsRaw
        .map((b) => {
    ...
    const features = padFeatures(featuresRaw
        .map((f) => {
        ...
        .filter((f) => Boolean(f)));
    const techRaw = nested.technical && typeof nested.technical === 'object'
        ? nested.technical
        : {};
    const metaRaw = nested.meta && typeof nested.meta === 'object' ? nested.meta : {};
    const marketRaw = nested.marketplace && typeof nested.marketplace === 'object'
        ? nested.marketplace
        : {};
    ...
    return {
        ...
        shortDescription: asString(nested.shortDescription || nested.short_description || nested.description, `${name} helps teams work faster with a focused SaaS workflow.`).slice(0, 800),
        problem: asString(nested.problem, 'Teams waste time on fragmented tools and manual processes.').slice(0, 1000),
        solution: asString(nested.solution, `${name} centralizes the workflow so teams can execute with less overhead.`).slice(0, 1000),
        featuresTagline: asString(nested.featuresTagline || nested.features_tagline, `Why teams choose ${name}`).slice(0, 200),
        featuresAboutBody: asString(nested.featuresAboutBody || nested.features_about_body || nested.about, `${name} is built for practical day-to-day use: clear workflows, sensible defaults, and room to customize when you need it.`).slice(0, 1500),
        benefits,
        features,
        ...
        bestFor: asString(nested.bestFor || nested.best_for, 'Growing teams that need a focused SaaS workflow').slice(0, 200),
        ...
        supportResponse: asString(nested.supportResponse || nested.support_response, 'Within 1 business day').slice(0, 120),
