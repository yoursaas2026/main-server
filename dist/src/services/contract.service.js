...
import { clients, contractAmendments, contractDisputes, contractEvents, contractPayments, contracts, developerProducts, } from '../db/schema.js';
...
export function computeContractAmounts(grossPaise) {
...
export function escrowCompletionSplit(escrowAmountPaise, platformCommissionPercent) {
...
export function contractCheckoutBreakdown(grossPaise, platformCommissionPercent) {
 ...
 */
export function resolveGrossPaiseFromListingTiers(customizationTiersJson, planTier, quotedGrossPaise) {
    const tierId = planTier.trim().toLowerCase();
    if (!tierId)
        throw new Error('Select a package (base, plus, or pro).');
    const tiers = parseJson(customizationTiersJson, []);
    const tier = tiers.find((t) => String(t.id || '').toLowerCase() === tierId);
    if (!tier) {
        throw new Error(`This listing does not define a “${tierId}” package. The seller sets Base / Plus / Pro pricing on the listing.`);
    }
    ...
    throw new Error('This Pro package is quoted per scope on the listing. Enter the agreed total in INR on the contract form, or ask the seller to set a fixed Pro price on the listing.');
}
async function logEvent(contractId, from, to, actorRole, actorId, meta) {
...
async function notify(contractId, text) {
    const [row] = await db
        ...
        .limit(1);
    if (row)
        await notifyContractParties(row.clientId, row.developerId, text);
}
export const contractService = {
    ...
    async listForClient(clientId, productId) {
    ...
    async listForDeveloper(developerId, productId) {
        const cond = productId
    ...
    async createContract(input) {
        ...
        const gross = resolveGrossPaiseFromListingTiers(product.customizationTiers, input.planTier, input.quotedGrossPaise ?? null);
        if (gross < 100)
        ...
        await notify(c.id, `📄 New contract #${publicId.slice(0, 8)} for "${product.name}" — pending your acceptance. Amount (incl. fees): ₹${(gross / 100).toFixed(2)}`);
        return c;
    },
    async developerAccept(contractId, developerId) {
        const [c] = await db.select().from(contracts).where(eq(contracts.id, contractId)).limit(1);
        if (!c || c.developerId !== developerId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.PENDING_DEVELOPER_ACCEPTANCE)
        ...
        await notify(contractId, `✅ Contract #${c.publicId.slice(0, 8)} accepted by developer. Please complete escrow payment to activate.`);
    },
    async developerReject(contractId, developerId) {
        const [c] = await db.select().from(contracts).where(eq(contracts.id, contractId)).limit(1);
        if (!c || c.developerId !== developerId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.PENDING_DEVELOPER_ACCEPTANCE)
    ...
    async clientCancelBeforeAccept(contractId, clientId) {
        const [c] = await db.select().from(contracts).where(eq(contracts.id, contractId)).limit(1);
        if (!c || c.clientId !== clientId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.PENDING_DEVELOPER_ACCEPTANCE)
            throw new Error('Only pending contracts can be cancelled this way');
    ...
    async createInitialEscrowOrder(contractPublicId, clientId) {
        const c = await this.getByPublicId(contractPublicId);
        if (!c || c.clientId !== clientId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.AWAITING_CLIENT_PAYMENT)
            throw new Error('Contract is not awaiting payment');
    ...
    async onEscrowPaymentCaptured(input) {
        ...
        if (pay.purpose === 'amendment' && pay.amendmentId) {
            ...
            if (!am || am.status !== 'awaiting_payment')
                return;
            await db
    ...
    async submitDeliverables(contractId, developerId, deliverablesText) {
        const [c] = await db.select().from(contracts).where(eq(contracts.id, contractId)).limit(1);
        if (!c || c.developerId !== developerId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.ACTIVE)
            throw new Error('Only active contracts can be submitted');
        ...
        await db
            ...
            .where(eq(contracts.id, contractId));
        await logEvent(contractId, c.status, next, 'developer', developerId, {});
        await notify(contractId, `📦 Developer submitted deliverables for #${c.publicId.slice(0, 8)}. Please accept, request revision, or open a dispute within ${days} days.`);
    },
    async clientAcceptCompletion(contractPublicId, clientId) {
        const c = await this.getByPublicId(contractPublicId);
        if (!c || c.clientId !== clientId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.SUBMITTED)
            throw new Error('Contract is not awaiting your decision');
        await this.settleCompleted(c.id);
    },
    async clientRequestRevision(contractPublicId, clientId, note) {
        const c = await this.getByPublicId(contractPublicId);
        if (!c || c.clientId !== clientId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.SUBMITTED)
    ...
    async clientOpenDispute(contractPublicId, clientId, reason) {
        const c = await this.getByPublicId(contractPublicId);
        if (!c || c.clientId !== clientId)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.SUBMITTED)
            throw new Error('Disputes can be opened after submission');
    ...
    async maybeAutoCompleteClientDeadline(contractId) {
        const [c] = await db.select().from(contracts).where(eq(contracts.id, contractId)).limit(1);
        if (!c || c.status !== ContractStatus.SUBMITTED || !c.clientDecisionDeadlineAt)
            return;
        if (new Date(c.clientDecisionDeadlineAt).getTime() > Date.now())
            return;
    ...
    async settleCompleted(contractId, reason = 'client_accepted') {
        ...
        if (c.status === ContractStatus.COMPLETED)
            return;
        if (c.status !== ContractStatus.SUBMITTED)
            throw new Error('Contract cannot be completed from this state');
        const platformPct = env.CONTRACT_PLATFORM_COMMISSION_PERCENT;
        const { developerPaise: devShare, platformFromEscrowPaise: platShare } = escrowCompletionSplit(c.escrowAmountPaise, platformPct);
    ...
    async proposeAmendment(input) {
        ...
        if (!c)
            throw new Error('Contract not found');
        if (c.status !== ContractStatus.ACTIVE)
            throw new Error('Amendments only while active');
        const proposedByClient = Boolean(input.actorClientId);
        if (proposedByClient && input.actorClientId !== c.clientId)
            throw new Error('Forbidden');
        if (!proposedByClient && input.actorDeveloperId !== c.developerId)
            throw new Error('Forbidden');
    ...
    async approveAmendment(amendmentId, actorClientId, actorDeveloperId) {
        const [am] = await db.select().from(contractAmendments).where(eq(contractAmendments.id, amendmentId)).limit(1);
        if (!am || am.status !== 'pending_counterparty')
        ...
        if (am.proposedByClient && actorDeveloperId !== c.developerId)
            throw new Error('Only developer can approve');
        if (!am.proposedByClient && actorClientId !== c.clientId)
            throw new Error('Only client can approve');
        if (am.additionalAmountPaise > 0) {
            ...
            return { needsPayment: true, orderId: order.orderId, amount: order.amount, currency: order.currency };
        }
        await db
            .update(contractAmendments)
    ...
    async verifyEscrowPayment(orderId, clientId) {
    ...
    async createAmendmentEscrowOrder(contractPublicId, clientId, amendmentId) {
        const c = await this.getByPublicId(contractPublicId);
        if (!c || c.clientId !== clientId)
            throw new Error('Contract not found');
        const [am] = await db
            ...
            .limit(1);
        if (!am || am.status !== 'awaiting_payment' || !am.paymentOrderId)
            throw new Error('Amendment not awaiting this payment');
    ...
    async resolveDispute(input) {
        ...
        if (total > c.escrowAmountPaise + c.nonRefundableFeePaise)
            throw new Error('Resolution amounts exceed recorded contract funds');
        await db
