...
async function saveUploadedFile(file, destFolder) {
    if (!file || typeof file === 'string')
        return null; // Ignore if not a File
    try {
...
export class DeveloperKycController {
    async submit(c) {
        try {
            ...
            const { country, documentType, documentNumber, frontImage, backImage, livenessVideo } = body;
            ...
            const frontFileUrl = await saveUploadedFile(frontImage, 'uploads/kyc');
            const backFileUrl = backImage ? await saveUploadedFile(backImage, 'uploads/kyc') : null;
            const livenessVideoUrl = await saveUploadedFile(livenessVideo, 'uploads/kyc');
            ...
            await db
                ...
                governmentIdNumber: documentNumber,
