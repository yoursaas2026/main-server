...
function mapProductRow(row, categoryName = '') {
    return {
        id: row.id,
    ...
    async list(c) {
        ...
        try {
            const whereExpr = pattern &&
                or(ilike(developerProducts.name, pattern), ilike(developerProducts.slug, pattern), ilike(developers.name, pattern), ilike(developers.email, pattern), ilike(developers.company, pattern));
            const baseQuery = db
                .select({
    ...
    async listCategories(c) {
        ...
        try {
            const rows = await db
                ...
                productCount: sql `count(${developerProducts.id})::int`,
            })
                .from(productCategories)
    ...
    async deleteCategory(c) {
        ...
        try {
            ...
            const [inUse] = await db
                ...
                .limit(1);
            if (inUse) {
                return c.json({ success: false, error: 'Category is currently used by one or more products' }, 409);
            }
