...
export class AdminMarketingUsersController {
    ...
    async create(c) {
        ...
        const [existing] = await db
            ...
            .limit(1);
        if (existing)
            return c.json({ success: false, error: 'Marketing user email already exists' }, 409);
        ...
        return c.json({
            success: true,
            message: 'Marketing user created. Create the same mailbox in Mailu admin with this email and password for webmail access.',
            data: { user: created },
        }, 201);
    }
    async update(c) {
        ...
        if (body.mailboxEmail)
            updateData.mailboxEmail = String(body.mailboxEmail).trim().toLowerCase();
        if (body.status)
            updateData.status = body.status;
        if (body.password)
            updateData.password = await hashPassword(String(body.password));
