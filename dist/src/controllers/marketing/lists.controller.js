...
export class MarketingListsController {
    ...
    async create(c) {
        ...
        if (!body?.name)
            return c.json({ success: false, error: 'List name is required' }, 400);
        const [list] = await db
    ...
    async remove(c) {
        ...
        if (!list)
            return c.json({ success: false, error: 'List not found' }, 404);
        if (list.isDefault)
            return c.json({ success: false, error: 'Cannot delete the default footer list' }, 400);
